...
object MyNetwork {
    ...
    class Request(internal val method: String) {
        ...
        private var jsonObjReqListener: MyJSONListener? = null
        private var myThreadExecutor: MyThreadExecutor = MyThreadExecutor()
        ...
        fun makeRequest(myJsonListener: MyJSONListener?): Request {
            this.jsonObjReqListener = myJsonListener
            myThreadExecutor.execute(RequestTask(this))
            return this
        }
    ...
    internal class RequestTask(private val req: MyNetwork.Request) : Runnable {
        override fun run() {
            try {
                val conn = request()
